...
class ECSContainerExporter(object):
    ...
    def collect_static_metrics(self):
        ...
        if metadata:
            self.log.debug(
                f"Using cached task metadata response from {self.cache_file_path}"
            )
        else:
            retries = 24
            while True and retries > 0:
                ...
                retries -= 1
                try:
                    metadata = utils.ecs_task_metdata(
                        self.task_metadata_url, self.http_timeout
                    )
                ...
                if metadata.get("KnownStatus") != "RUNNING":
                    self.log.warning(
                        f'ECS Task not yet in RUNNING state, current status is: {metadata["KnownStatus"]}'
                    )
                    continue
                else:
                    break
    ...
    def parse_task_metadata(self, metadata):
        ...
        self.task_cpu_limit, self.task_mem_limit = self.cpu_mem_limit(
            metadata, normalise_cpu=True
        )

        metric = create_metric(
            "cpu_limit", self.task_cpu_limit, task_tag, "gauge", "Task CPU limit"
        )
        self.static_task_metrics.append(metric)

        metric = create_metric(
            "mem_limit", self.task_mem_limit, task_tag, "gauge", "Task Memory limit"
        )
        self.static_task_metrics.append(metric)
        ...
        for container in metadata["Containers"]:
            ...
            if self.should_process_container(
                container_name, self.include_containers, self.exclude_containers
            ):
                self.log.info(
                    f"Processing stats for container: {container_name} - {container_id}"
                )
                self.include_container_ids.append(container_id)
            else:
                self.log.info(
                    f"Excluding container: {container_name} - {container_id} as per exclusion"
                )
            ...
            self.task_container_limits[container_id] = {
                "cpu": cpu_value,
                "mem": mem_value,
            }
            ...
            if container_id in self.include_container_ids:
                metric = create_metric(
                    "cpu_limit",
                    cpu_value,
                    self.task_container_tags[container_id],
                    "gauge",
                ...
                metric = create_metric(
                    ...
                    self.task_container_tags[container_id],
                    "gauge",
                    "Limit in memory usage in MBs",
                )
                self.static_task_metrics.append(metric)

    def should_process_container(
        self, container_name, include_containers, exclude_containers
    ):
    ...
    def collect_all_metrics(self):
        ...
        metric = create_metric(
            "exporter_status", self.exporter_status, {}, "gauge", "Exporter Status"
        )
    ...
    def collect_container_metrics(self):
        metrics = []
        # Only keep the last sample for the docker stats
        self.docker_stats = self.docker_stats[-1:]
        try:
            self.docker_stats.append(
                utils.ecs_task_metdata(self.task_stats_url, self.http_timeout)
            )
            if not all(self.docker_stats):
        ...
        container_metrics_all = self.parse_container_metadata(
            self.task_cpu_limit,
            self.task_container_limits,
            self.task_container_tags,
        )
    ...
    def parse_container_metadata(
        self, task_cpu_limit, task_container_limits, task_container_tags
    ):
        """
        ...
        for container_id in list(self.docker_stats[1].keys()):
            if not self.docker_stats[1][container_id]:
                del self.docker_stats[0][container_id]
                del self.docker_stats[1][container_id]
        ...
        try:
            ...
            # Memory metrics
            container_metrics_all.append(
                calculate_memory_metrics(self.docker_stats[1], task_container_tags)
            )

            # I/O metrics
            container_metrics_all.append(
                calculate_io_metrics(self.docker_stats[1], task_container_tags)
            )

            # network metrics
            container_metrics_all.append(
                calculate_network_metrics(self.docker_stats[1], task_container_tags)
            )

        except Exception as e:
            self.log.warning(
                "Could not retrieve metrics for {}: {}".format(task_container_tags, e),
                exc_info=True,
            )
            self.exporter_status = 1
...
@click.option(
    "--metadata-url",
    envvar="ECS_CONTAINER_METADATA_URI",
    type=str,
    default=None,
...
def main(
    ...
    if not metadata_url:
        sys.exit(
            "AWS environment variable ECS_CONTAINER_METADATA_URI not found "
    ...
    collector = ECSContainerExporter(
